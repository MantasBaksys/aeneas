...
-- computable model of Iterator::map+collect using the FnMut dictionary
...
mutual
  partial def map_tree (t : Tree) : Result Tree :=
    ...
    | .node v => do
        let v' ← mapList
          ({ FnOnceInst := { call_once := map_tree.closure.call_once },
             call_mut := map_tree.closure.call_mut } : FnMut Unit Tree Tree) () v
        ok (.node v')
...
def map_tree.closure.FnOnceInst : FnOnce Unit Tree Tree :=
  { call_once := map_tree.closure.call_once }
def map_tree.closure.FnMutInst : FnMut Unit Tree Tree :=
  { FnOnceInst := map_tree.closure.FnOnceInst, call_mut := map_tree.closure.call_mut }

#check @map_tree
#check @map_tree.closure.FnMutInst
#eval (match map_tree (.node [.leaf 1, .node [.leaf 2]]) with | .ok _ => "ok" | _ => "other")
